<#macro componentInput item>
    <#if item.type='date'>
        <el-date-picker v-model="form.${item.id!}" type="date" value-format="YYYY-MM-DD  HH:mm:ss" placeholder="选择日期">
        </el-date-picker>
    <#elseif item.type='el_date_picker'>
        <el-date-picker v-model="form.${item.id!}" type="date" value-format="YYYY-MM-DD  HH:mm:ss" placeholder="选择日期">
        </el-date-picker>
    <#elseif item.type='el_date_time_picker'>
        <el-date-picker v-model="form.${item.id!}" type="datetime" value-format="YYYY-MM-DD  HH:mm:ss"
                        placeholder="选择日期">
        </el-date-picker>
    ...
    <#elseif item.type='tree'>
        <el-tree-select ref="treeSelect" v-model="form.${item.id!}" :data="${item.id!}Options" node-key="id" check-strictly=true :render-after-expand="false" />
    ...
    <#elseif item.type='data_select'>
        <data-select catalog="${item.id!}List" v-model="form.${item.id!}"></data-select>
